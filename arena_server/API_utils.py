...
class API_Interface:
    ...
    def make_request(self, url, method, data=None):
        ...
        try:
            response = requests.request(
                method=method, url=url, headers=headers, data=data, timeout=200
            )
    ...
    def update_submission_data(self, data, challenge_pk):
        ...
        url = self.return_url_per_environment(url=url)
        response = self.make_request(url=url, method="PUT", data=data)
        return response
