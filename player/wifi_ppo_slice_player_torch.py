...
from player.deep_neural_network_model_slice_torch import ResNet18, ResNet34, ResNet50, ResNet101, ResNet152
from player.deep_neural_network_model_torch_simple import DnnModel
...
class PPOPlayer(Player):
    ...
    def connect_to_server(self, server_address: str, server_port: int):
        ...
        self._main_dnn = ResNet18(self._num_freq_channel, self._max_num_unit_packet).to(self._device)
        # self._main_dnn = DnnModel(self._num_freq_channel, self._observation_history_length,
        #                           self._num_action).to(self._device)
        # summary(self._main_dnn, input_size=(2, self._observation_history_length, self._num_freq_channel))
        self._model_optimizer = torch.optim.Adam([{'params': self._main_dnn.parameters(),
                                                   'lr': self._dnn_learning_rate}])
        self._policy_old = ResNet18(self._num_freq_channel, self._max_num_unit_packet).to(self._device)
    ...
    def train_dnn(self, num_episodes: int, replay_memory_size: int, mini_batch_size: int, dnn_epochs: int,
        ...
        for episode in range(num_episodes):
            ...
            observations, channels, packets, logprobs, discounted_rewards, old_values = self.get_mini_batch(mini_batch_size)
            observations = torch.squeeze(observations).detach().to(self._device)
            actions = torch.squeeze(channels).detach().to(self._device)
            packets = torch.squeeze(packets).detach().to(self._device)
            ...
            for epoch in range(dnn_epochs):
                ...
                action_logprobs, values = self.get_evaluation_action(observations, channels,
                                                                                           packets)
    ...
    def total_loss(self, logprobs, old_logprobs, values, discounted_rewards, critic_loss):
    ...
    def get_evaluation_action(self, observation: np.ndarray, channels, packets):
        """
        ...
        """
        channel_probs, packet_probs, values = self._main_dnn(observation)
        channel_distribution = Bernoulli(probs=channel_probs)
        packet_distribution = Categorical(probs=packet_probs)
        channel_logprobs = channel_distribution.log_prob(channels)
        packet_logprobs = packet_distribution.log_prob(packets)
        # action_logprobs = torch.sum(torch.Tensor([torch.sum(channel_logprobs, dim=1), packet_logprobs]),
        #                             dim=0)
        action_logprobs = torch.sum(channel_logprobs, dim=1) + packet_logprobs
        # distribution_entropy = channel_distribution.entropy() + packet_distribution.entropy()
    ...
    def get_next_action(self, observation: np.ndarray):
        ...
        channel_probs, packet_probs, value = self._policy_old(observation)
        channel_distribution = Bernoulli(probs=channel_probs)
        packet_distribution = Categorical(probs=packet_probs)
        channel = channel_distribution.sample()
        packet = packet_distribution.sample()
        channel_logprob = channel_distribution.log_prob(channel)
        packet_logprob = packet_distribution.log_prob(packet)
        action_logprob = torch.sum(torch.Tensor([[torch.sum(channel_logprob, dim=1), packet_logprob]]),
                                   dim=1)
        return channel[0].detach(), packet.detach(), action_logprob.detach(), value
    ...
    def accumulate_replay_memory(self, replay_memory_size: int, progress_report: bool):
        ...
        for ind in range(replay_memory_size):
            ...
            with torch.no_grad():
                ...
                channel, packet, action_logprob, value = self.get_next_action(prev_observation_history)
            packet_index = int(packet)
            action_dict = self.convert_action_index_to_dict(channel, packet_index)
            ...
            with torch.no_grad():
                ...
                _, _, _, next_value = self.get_next_action(current_observation_history)
            experience = (prev_observation_history, channel, packet, reward, action_logprob, value, next_value)
    ...
    def test_run(self, length: int):
        ...
        for ind in range(length):
            ...
            with torch.no_grad():
                prev_observation_history = torch.FloatTensor(prev_observation_history).to(self._device)
                channel, packet, _, _ = self.get_next_action(prev_observation_history)
            packet_index = int(packet)
            action_dict = self.convert_action_index_to_dict(channel, packet_index)
            observation_dict = self.step(action_dict)
            self.update_observation_history(action_dict, observation_dict)
    ...
    def get_mini_batch(self, batch_size: int):
        ...
        observation = torch.stack([x[0] for x in samples], dim=0)
        channels = torch.stack([x[1] for x in samples], dim=0)
        packets = torch.stack([x[2] for x in samples], dim=0)
        rewards = np.stack([x[3] for x in samples], axis=0)
        logprobs = torch.stack([x[4] for x in samples], dim=0)
        values = torch.stack([x[5] for x in samples], dim=0)
        next_values = torch.stack([x[6] for x in samples], dim=0)
        discount_factors = []
        for ind, sample in enumerate(samples):
            channel = sample[1]
            packet = sample[2]
            action_dict = self.convert_action_index_to_dict(channel, packet)
        ...
        return observation, channels, packets, logprobs, discounted_rewards, values
    ...
    def convert_action_index_to_dict(self, channel, packet) -> Dict:
        ...
        """
        freq_channel = list(map(int, channel.to('cpu')))
        if np.nansum(freq_channel) == 0:
            ...
            action_dict = {'type': 'tx_data_packet', 'freq_channel_list': freq_channel,
                           'num_unit_packet': packet}
        return action_dict
