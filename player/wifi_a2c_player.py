...
from player.actor_critic_nn_model import Actor, Critic
...
class A2CPlayer(Player):
    def __init__(self, identifier: str, max_num_unit_packet: int, observation_history_length: int,
        ...
        super(A2CPlayer, self).__init__(identifier)
        ...
        self._actor: Optional[tf.keras.Model] = None
        self._critic: Optional[tf.keras.Model] = None
        self._actor_optimizer: Optional[tf.keras.optimizers] = None
    ...
    def connect_to_server(self, server_address: str, server_port: int):
        super(A2CPlayer, self).connect_to_server(server_address, server_port)
        ...
        self._actor = Actor(self._dnn_layers_list, self._num_action)
        self._critic = Critic(self._dnn_layers_list, 1)
        self._actor_optimizer = Adam(lr=self._dnn_learning_rate)
        self._critic_optimizer = Adam(lr=self._dnn_learning_rate)

    def actor_loss(self, probabilities, action, temporal_difference):
        distribution = tfp.distributions.Categorical(probs=probabilities, dtype=tf.float32)
        log_probability = distribution.log_prob(action)
        loss = -log_probability * temporal_difference
        return loss

    def train_dnn(self, num_episodes: int, dnn_epochs: int, progress_report: bool, time_step: int,
                  test_run_length: int):
        for episode in range(num_episodes):
            if progress_report:
                print(f"Episode: {episode}")
            for epoch in range(dnn_epochs):
                self.compute_gradient(time_step, progress_report)
    ...
    def get_next_action(self, observation: np.ndarray):
        action_prob = self._actor(np.array([observation]))
    ...
    def compute_gradient(self, time_step: int, progress_report: bool):
        for ind in range(time_step):
            if progress_report:
                print(f"Replay memory sample: {ind}/{time_step}\r", end='')
            ...
            prev_observation_history = np.array([prev_observation_history])
            with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
                policy = self._actor(prev_observation_history, training=True)
                value = self._critic(prev_observation_history, training=True)
                next_value = self._critic(np.array([self._observation_history]), training=True)
                temporal_difference = reward + self._gamma * next_value - value
                actor_loss = self.actor_loss(policy, action_index, temporal_difference)
                critic_loss = temporal_difference ** 2
            actor_gradient = tape1.gradient(actor_loss, self._actor.trainable_variables)
            critic_gradient = tape2.gradient(critic_loss, self._critic.trainable_variables)
            self._actor_optimizer.apply_gradients(zip(actor_gradient, self._actor.trainable_variables))
    ...
    def model_save(self, scenario: int, modelNumber: int, episode: int, CSV: bool = False):
        ...
        self._actor.save(path+"actor_episode_%d" % episode)
