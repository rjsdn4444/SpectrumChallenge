...
class PPOPlayer(Player):
    ...
    def train_dnn(self, num_episodes: int, replay_memory_size: int, mini_batch_size: int, dnn_epochs: int,
        ...
        for episode in range(num_episodes):
            ...
            observations, actions, _, discounted_rewards, _ = self.get_mini_batch(mini_batch_size)
            ...
            for epoch in range(dnn_epochs):
                ...
                total_loss = self.total_loss(action_logprobs, values, discounted_rewards,
    ...
    def total_loss(self, logprobs, values, discounted_rewards, distribution_entropy, critic_loss):
        values = torch.squeeze(values)
        advantages = discounted_rewards - values.detach()
        policy_loss = (-logprobs * advantages).sum()
        total_loss = policy_loss + self._value_function_coef * critic_loss + self._entropy_beta * distribution_entropy
        return total_loss

    def critic_loss(self, values, discounted_rewards):
        values = torch.squeeze(values)
        loss = (values - discounted_rewards).pow(2)
        critic_loss = 0.5 * loss.sum()
        return critic_loss
    ...
    def get_mini_batch(self, batch_size: int):
        ...
        actions = torch.stack([x[1] for x in samples], dim=0)
        rewards = np.stack([x[2] for x in samples], axis=0)
        logprobs = torch.stack([x[3] for x in samples], dim=0)
        values = torch.stack([x[4] for x in samples], dim=0)
        next_values = torch.stack([x[5] for x in samples], dim=0)
        discount_factors = []
